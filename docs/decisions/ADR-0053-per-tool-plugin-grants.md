...
**Status:** Accepted 2026-05-12. T1 + T2 + T3 + T4 shipped in
Bursts 235 + 237 + 238 + 239. **The dispatcher now honors per-tool
grants end-to-end** — operators creating per-tool grants via the
T3 endpoints actually shape mcp_call.v1 dispatch decisions via the
T4 specificity-wins resolver. T5 (frontend UI) + T6 (cross-doc
updates) queued. Pairs with ADR-0048 (Computer Control Allowance) —
