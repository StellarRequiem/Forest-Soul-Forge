...
| T4 | Allowance UI | **DONE B165 + B240 (fully)** — Chat-tab assistant settings panel ships three preset buttons (Restricted / Specific / Full) wired to POST/DELETE `/agents/{id}/plugin-grants` + the per-tool routes added by ADR-0053 T3. **B240 (ADR-0053 T5) closed the per-tool granularity gap** the original B165 row called out: the Advanced disclosure went from read-only reference table to interactive per-tool checkbox grid wired to the new per-tool endpoints (POST with `tool_name` + DELETE on the new `/tools/{tool_name}` path). Preset semantics now follow ADR-0053 D5: Restricted clears all grants (plugin-level + per-tool); Specific revokes the plugin-level grant and seeds per-tool yellow grants for the two read_only tools (operator extends via Advanced); Full revokes any per-tool overrides and issues a plugin-level grant at green tier. Unchecking a tool currently covered by the plugin-level grant records a per-tool RED grant — the "carve out a denial inside a broad grant" use case ADR-0053 D2 calls out, resolved at dispatch by the ADR-0053 T4 specificity-wins resolver (B239). Coverage column distinguishes per-tool overrides from plugin-level coverage so the operator sees what's narrowing what. Closes ADR-0047 T4 fully (the allowances stub is gone). | shipped |
