...
- **D9 Learning Coach** — **Phases A + B + C IN FLIGHT (2026-05-23).** ADR-0089 Phases A + B + C: mentor + curriculum_designer (researcher, GREEN, Phase A) + assessor (guardian, YELLOW, Phase B) + socratic_partner (communicator, GREEN, Phase C) live in trait_tree / genres / constitution_templates / tool_catalog. Four new builtin tools across Phases A+B: `curriculum_design.v1` (read_only; 27 tests; deterministic topic-prereq DAG composer), `knowledge_assessment.v1` (read_only; 19 tests; quiz-item envelope generator), `assessment_score.v1` (read_only; 17 tests; strict-match + Jaccard lexical-overlap scorer), `misconception_log.v1` (filesystem; 18 tests; misconception ledger writer — NOT in any role's standard kit because guardian's read_only ceiling rejects filesystem; operator dispatches directly post-review per the D1 knowledge_verifier separation pattern). Phase C ships no new builtin tools — socratic_partner reuses existing kit. Five skill manifests (`coaching.v1`, `curriculum_design.v1`, `knowledge_assessment.v1`, `misconception_tracking.v1`, `socratic_dialogue.v1`). Birth scripts for mentor/curriculum_designer/assessor/socratic_partner. Runbook `docs/runbooks/d9-learning-coach-ops.md`. ADR-0089 in Proposed status. Phase D queued (spaced_repetition_pilot YELLOW + 1 tool + cascade wiring + umbrella → ADR Accepted + domain live).
- **D10/D5/D6** — upstream; not started. (D9 is in flight per ADR-0067 rollout order.)
