...
- **D2 Daily Life OS** — **Phase A IN FLIGHT (2026-05-23).** ADR-0087 Proposed. Phase A roles: coordinator (researcher, GREEN) + inbox_triager (communicator, GREEN). No new builtin tools in Phase A — reuses existing kit. Two skills: `daily_orchestration.v1` + `inbox_triage.v1`. Birth scripts: `dev-tools/birth-coordinator.command` + `dev-tools/birth-inbox-triager.command`. Runbook `docs/runbooks/d2-daily-life-os-ops.md`. Phases B/C/D queued: time_steward (YELLOW, actuator) with schedule_reminder.v1 + calendar_block.v1; task_prioritizer with task_rank.v1; reflector with decision_journal_compile.v1. Cascade d1→d2 morning_briefing activates at Phase D.
