...
- **D9 Learning Coach** — **Phase A IN FLIGHT (2026-05-23).** ADR-0089 Phase A: mentor + curriculum_designer roles (both researcher genre, GREEN) live in trait_tree / genres / constitution_templates / tool_catalog. One new builtin tool: `curriculum_design.v1` (read_only; 27 tests; deterministic topic-prereq DAG composer with stable tie-breaking — fewer-unmet-prereqs > shallower depth > catalog index — so two calls with the same inputs always produce the same path). Two skill manifests (`coaching.v1`, `curriculum_design.v1`). Birth scripts `dev-tools/birth-mentor.command` + `dev-tools/birth-curriculum-designer.command`. Runbook `docs/runbooks/d9-learning-coach-ops.md`. ADR-0089 in Proposed status. Phases B/C/D queued (B: assessor YELLOW + 3 tools; C: socratic_partner reuses kit; D: spaced_repetition_pilot YELLOW + 1 tool + cascade wiring + umbrella).
