...
Last updated: 2026-05-04, post-Burst 108 (v0.5.0-rc ready to tag). v0.5 arc covers two Accepted ADRs: **ADR-0042 v0.5 Product Direction** (Tauri installer / SMB-prosumer thesis / PWA-first mobile / local-only-free / single repo — T1 doc Burst 97, T2 responsive Burst 98, T3.1 Tauri shell Burst 99, T4 PyInstaller binary Burst 101; T5 signing/auto-updater gated on Apple Developer account decision) and **ADR-0043 MCP-First Plugin Protocol** (T1 design Burst 103, T2 manifest schema + `fsf plugin` CLI Burst 104, T3 daemon hot-reload + /plugins endpoints Burst 105, T4 audit-chain integration Burst 106, T4.5 dispatcher bridge Burst 107, T5 example plugins + contribution guide Burst 108 — implementation-complete; per-tool approval mirroring + allowed_mcp_servers auto-grant + frontend Tools-tab plugin awareness + plugin_secret_set audit event documented as deferred follow-ups). Test count 2177 → 2289 across the v0.5 arc (+112, zero regressions). v0.4.0 shipped 2026-05-04 (ADR-0041 Set-and-Forget Orchestrator complete). v0.3.0 shipped 2026-05-03 (ADR-0036 Verifier Loop + ADR-0040 Trust-Surface Decomposition). v0.2.0 shipped 2026-05-02 (Phase G.1.A — 10 programming primitives). v0.1.2 shipped 2026-05-01 absorbing SarahR1's review (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30 (audit + hardening). See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
...
| | |
|---:|:---|
| Source LoC (Python) | **48,760** across `src/forest_soul_forge/` (44,648 at v0.4.0 + ~4,100 across the v0.5 arc — ADR-0042 PyInstaller harness + ADR-0043 plugins package + daemon /plugins router + dispatcher bridge + manifest validation suite). |
| Tests (passing) | **2289** (was 1567 at v0.1.2; +722 net — +401 across the v0.2 arc, +104 across the v0.3 ADR-0036 arc, +105 across the v0.4 ADR-0041 arc, +112 across the v0.5 ADR-0042/0043 arc Bursts 95-108) |
| ADRs filed | **40 files / 38 unique numbers** (`ADR-0001` → `ADR-0043`, with gaps 0009-0015; ADR-003X open-web + ADR-003Y conversation runtime drafts; ADR-0021-am + ADR-0027-am amendments + ADR-0038 all Accepted in v0.1.2; ADR-0035 Persona Forge + ADR-0037 Observability dashboard Proposed for v0.3; ADR-0036 Verifier Loop feature-complete in v0.3; ADR-0039 Distillation Forge Proposed for v0.4; ADR-0040 Trust-surface decomposition Accepted v0.3.0; ADR-0041 Set-and-Forget Orchestrator Accepted v0.4.0; **ADR-0042 v0.5 Product Direction Accepted (T1+T2+T3.1+T4 shipped; T5 signing/auto-updater gated)**; **ADR-0043 MCP-First Plugin Protocol Accepted (T1+T2+T3+T4+T4.5+T5 implementation-complete; per-tool approval mirroring + allowed_mcp_servers auto-grant + frontend awareness + plugin_secret_set deferred follow-ups)**) |
...
| Schema version | **v13** (v8: agent_secrets / v9: memory_verifications / v10: conversations / v11: epistemic memory — claim_type + confidence + last_challenged_at + memory_contradictions table from ADR-0027-am T1; v12: flagged_state column on memory_contradictions for ADR-0036 T6; v13: scheduled_task_state for ADR-0041 T5 scheduler persistence). v0.5 arc added zero schema changes — plugin state lives on disk under `~/.forest/plugins/`, not in the registry DB. |
...
| Audit event types | **67** (54 pre-v0.3 + verifier_scan_completed v0.3 + 7 ADR-0041 scheduler events v0.4 + **5 ADR-0043 plugin lifecycle events v0.5**: plugin_installed / plugin_uninstalled / plugin_enabled / plugin_disabled / plugin_verification_failed). plugin_secret_set deferred per ADR-0043 follow-ups. |
...
| `.command` operator scripts | **120** at repo root (107 at v0.4.0 + 13 across the v0.5 arc Bursts 95-108 — commit-burst95 through 108 + tag-v0.5.0-rc helpers). Includes start/stop/reset + live-tests + commit-burst* + dist/build + ops scripts. |
...
| Total commits on `main` | **264** (250 at v0.4.0 + 14 across the v0.5 arc Bursts 95-108: v0.4.0 docs/tag + v0.5 planning + ADR-0042 + responsive + Tauri shell + responsive audit + PyInstaller + audience README + ADR-0043 + plugin CLI + daemon runtime + audit integration + dispatcher bridge + example plugins + audit-chain refresh) |
