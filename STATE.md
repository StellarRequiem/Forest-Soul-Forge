...
- **D1 Personal Knowledge Forge** — **Phase A in flight (2026-05-23).** ADR-0086 Proposed. Phase A roles `librarian` (guardian) + `prospector` (researcher) added to trait_tree / genres / constitution_templates / tool_catalog; birth scripts `dev-tools/birth-librarian.command` + `dev-tools/birth-prospector.command`; skill manifests `knowledge_curation.v1` + `research_gathering.v1`; runbook `docs/runbooks/d1-knowledge-forge-ops.md`. Phases B–D queued: synthesizer + topic_genealogy_build.v1 (B), knowledge_verifier + knowledge_contradiction_scan.v1 (C, YELLOW), daily_knowledge_delta.v1 + cascade wiring (D). Manifest's bare `verifier` role renamed to `knowledge_verifier` per ADR-0086 Decision 2 to avoid collision with kernel-singleton verifier_loop + reality_anchor.
- **D2/D7/D9/D10/D5/D6** — upstream; not started.
