...
- **D8 Compliance Auditor** — ROLLING OUT (ADR-0085, started 2026-05-22). Phase A CLOSED — audit_archivist + evidence_collector alive. Phase B CLOSED — compliance_scanner + `framework_check.v1` (25 unit tests) + SOC2 seed framework. Phase C CLOSED — policy_enforcer (actuator genre, YELLOW posture) + `policy_lint.v1` builtin (20 unit tests) + SOC2 `lint_rules` section + `policy_enforcement.v1` skill + birth script. Phase D (report_generator + audit_packet_generate.v1 + cascade) pending.
