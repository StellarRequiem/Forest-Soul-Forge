...
- **D8 Compliance Auditor** — ROLLING OUT (ADR-0085, started 2026-05-22). Phase A CLOSED — audit_archivist + evidence_collector roles + skills (`evidence_collection.v1` + `long_term_archival.v1`) + birth scripts shipped; AuditArchivist-D8 + EvidenceCollector-D8 alive. Phase B CLOSED — compliance_scanner role + `framework_check.v1` builtin tool (25 unit tests) + SOC2 seed framework (`config/compliance_frameworks/soc2.yaml`) + `compliance_scan.v1` skill + birth script. Phase C (policy_enforcer + policy_lint.v1) + Phase D (report_generator + audit_packet_generate.v1 + cascade) pending.
- **D1/D2/D7/D9/D10/D5/D6** — upstream; not started.
