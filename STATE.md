...
Last updated: 2026-05-12, post-Burst 233 (marketplace seed skills 6-10 — 20-item seed catalog complete). Bursts 213-233 layered four overlapping arcs onto the post-B212 baseline. (1) **Swarm re-acceptance + 24/7 ops** (B213-B216): STATE.md refresh, ADR-0033 canonical Security Swarm chain re-fired live with the auth-header quoting fix at `seqs 7593-7643` (3 `agent_delegated` hops at 7612/7621/7630), and Mac mini 24/7 launchd LaunchAgents installed so the daemon + scheduler survive logout/reboot. (2) **ADR-0054 T6 runbook** (B217) shipped the procedural-shortcut-dispatch safety guide at `docs/runbooks/procedural-shortcuts.md`. (3) **ADR-0060 Runtime Tool Grants — full arc** (B218-B223): a new `agent_catalog_grants` table (schema v17) lets operators grant a single builtin tool to a running agent without rebirthing it, mirroring the existing per-plugin grant surface. T1 schema, T2 dispatcher integration, T3 `/agents/{id}/tools/grant` endpoints, T4 posture × trust_tier 9-cell interaction matrix (3 new gate_source values: `posture_clamp_with_grant`, `grant_overrides_posture`, `grant_blocked_by_red_posture`), T5 28 unit tests across both interaction tables, T6 frontend grants pane on the Agents tab. (4) **MCP universality + agentic marketplace Phase A** (B224-B233): `mcp_call.v1` got real `http://`+`https://` transport with `auth_header_template` support (B225, ten new tests); ADR-0055 expanded with D8-D11 + M7-M10 (reviews, telemetric scores, agent templates, marketplace auditability) plus a 4-phase rollout roadmap at `docs/roadmap/2026-05-11-marketplace-roadmap.md` (B226); Phase A shipped end-to-end — `POST /marketplace/install` with tarball-traversal defense (B227), frontend Marketplace tab + Browse pane with side_effects filter (B228), post-install grant-to-agent flow (B229); and the **20-item marketplace seed catalog** landed in `examples/tools/` (10 prompt-template tools — B230-B231) and `examples/skills/` (10 multi-step composition skills — B232-B233), all 20 validated by the kernel's own `parse_spec`/`parse_manifest` with post-B208 ref-scope coverage. The catalog is the seed content the `forest-marketplace` sibling repo will index as its first entries once Alex scaffolds the GitHub org. v0.5 ADRs remain canonical: **ADR-0042**, **ADR-0043**, **ADR-0045**. New since B212: **ADR-0060 Runtime Tool Grants** (Accepted, all six tranches shipped). Test count post-B233: **2,800 unit + integration passing** (sandbox baseline; +62 over post-B212 B225+B222 test additions). Latest tag still **v0.5.0** (2026-05-04). v0.6 has not yet been tagged — gated on integrator validation per ADR-0044 P7. v0.4.0 shipped 2026-05-04 (ADR-0041). v0.3.0 shipped 2026-05-03 (ADR-0036 + ADR-0040). v0.2.0 shipped 2026-05-02 (Phase G.1.A). v0.1.2 shipped 2026-05-01 (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30. See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
