...
<!-- CANON:BEGIN — generated by dev-tools/state_canon.py --emit on 2026-06-04 03:13Z. Do not hand-edit; run `--check` to verify against disk. -->

| Surface | Canon (generated 2026-06-04 03:13Z) |
|---:|:---|
| Python LoC (`src/forest_soul_forge/`) | **102,201** |
| ADRs filed | **91** files / **89** unique numbers |
| Builtin tools | **100** |
| Tests | **5,249** `def test_` across **266** files _(structural count — suite GREEN/pass-count is a CI artifact, not asserted here)_ |
...
| As of | **`336de14`** · **697** commits on `main` _(VCS provenance — advances every commit)_ |
| Registry agents | **67 active / 5 archived / 72 total** _(local runtime DB — gitignored, absent in CI)_ |
| Audit chain entries | **27,840** at `examples/audit_chain.jsonl` _(grows continuously at runtime — snapshot only)_ |
