...
- **D1 Personal Knowledge Forge** — **Phase B closed (2026-05-23).** ADR-0086 Phases A+B CLOSED. Roles live: `librarian` (guardian), `prospector` (researcher), `synthesizer` (researcher). New builtin tool: `topic_genealogy_build.v1` (28 unit tests). Four skill manifests: `knowledge_curation.v1`, `research_gathering.v1`, `topic_genealogy.v1`, `knowledge_summarize.v1`. Three birth scripts: `birth-librarian` / `birth-prospector` / `birth-synthesizer`. Runbook `docs/runbooks/d1-knowledge-forge-ops.md` (Phase A + B sections). Phases C–D queued: knowledge_verifier + knowledge_contradiction_scan.v1 (C, YELLOW), daily_knowledge_delta.v1 + cascade wiring (D). Manifest's bare `verifier` role renamed to `knowledge_verifier` per ADR-0086 Decision 2.
