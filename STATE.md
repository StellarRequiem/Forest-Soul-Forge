...
- **D6 Personal Finance Guardian** — **IN FLIGHT — Phases A + B + C shipped (2026-05-24).** ADR-0092 Phases A + B + C landed. Roles: budget_analyst (researcher, GREEN) + risk_advisor (guardian, GREEN) [Phase A]; transaction_tracker (researcher, GREEN) + bill_steward (researcher, GREEN) [Phase B]; investment_researcher (researcher, GREEN) [Phase C]. Three new builtin tools: `transaction_categorize.v1` (read_only; 44 tests; deterministic rule-based categorization over operator-supplied transaction batches with first-match-wins rule application; merchant/description/amount predicates), `bill_recurrence_check.v1` (read_only; 46 tests; deterministic recurrence detection for monthly/quarterly/annual cycles with operator-tolerable day_drift + next-due projection + missing-cycle flagging), `investment_compare.v1` (read_only; 45 tests; deterministic side-by-side comparison composer; per-dimension winners + per-option deltas across lower_is_better / higher_is_better / info_only dimension classes; NEVER advises which to pick — operator decides). Five skill manifests (`budget_analysis.v1`, `risk_analysis.v1`, `transaction_monitoring.v1`, `bill_management.v1`, `investment_research.v1`). Birth scripts for all five roles under `dev-tools/birth-*-d6.command`. Runbook `docs/runbooks/d6-finance-ops.md`. Phase D (cascade + umbrella + live) pending. D6's hard rule: NEVER executes transactions (manifest contract). ALL D6 roles are GREEN — first all-GREEN domain in the rollout, because the actuating surface (bank, broker, bill-pay) is operator-only by manifest contract; there is no actuator lane to define. Closes the rollout per ADR-0067 (D6 is the LAST domain).
