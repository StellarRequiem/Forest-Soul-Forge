...
## ⏱️ CANON — machine-generated current truth

> Generated by [`dev-tools/state_canon.py`](dev-tools/state_canon.py) (`--emit`) and verified by `--check` (the drift gate). **Everything below the CHRONICLE divider is historical and fenced as SUPERSEDED — do not quote it as current.** Structural facts are measured from disk exactly; `schema version` is operator-declared (the daemon `/healthz` is the live check); the test pass-count / GREEN status is a CI artifact, deliberately not asserted here. Model provider: **local Ollama** `qwen3:8b` (classify / generate / safety_check / conversation / tool_use) + `nomic-embed-text`.

<!-- CANON:BEGIN — generated by dev-tools/state_canon.py --emit on 2026-06-04 07:37Z. Do not hand-edit; run `--check` to verify against disk. -->

| Surface | Canon (generated 2026-06-04 07:37Z) |
|---:|:---|
| Python LoC (`src/forest_soul_forge/`) | **102,953** |
| ADRs filed | **93** files / **91** unique numbers |
| Builtin tools | **101** |
| Tests | **5,288** `def test_` across **272** files _(structural count — suite GREEN/pass-count is a CI artifact, not asserted here)_ |
| Latest tag / pyproject version | **v0.5.0** / **0.5.0** |
| Schema version | **v23** _(operator-declared; not static-measurable — daemon `/healthz` is the live check)_ |
| _— below: informational, not gated —_ | |
| As of | **`4d16785`** · **708** commits on `main` _(VCS provenance — advances every commit)_ |
| Registry agents | **67 active / 5 archived / 72 total** _(local runtime DB — gitignored, absent in CI)_ |
| Audit chain entries | **28,016** at `examples/audit_chain.jsonl` _(grows continuously at runtime — snapshot only)_ |

<!-- CANON:END -->

---

## 📜 CHRONICLE — superseded snapshots (historical; DO NOT quote as current)

> The blocks below were canon at their stamped burst and are preserved for audit lineage only. **Every figure in them is superseded by the CANON block above.** They are fenced precisely because an external reviewer once quoted a stale number from this chronicle (`59,602 LoC / 2,800 tests / schema v20 / "tamper-PROOF"`) as if it were present truth — the exact failure this structure exists to prevent.

_The 2026-05-19 (Burst 420) snapshot below was the reference for figures not re-measured during the 2026-06-01 realignment; the CANON block above now supersedes all of it._
...
> ⛔ **SUPERSEDED — historical snapshot, as of 2026-05-19 (Burst 420). DO NOT quote any figure below as current.** Superseded by the CANON block at the top of this file. Kept for audit lineage.
...
> ⛔ **SUPERSEDED — historical narrative, as of 2026-05-13 (Burst 258). DO NOT quote any figure below as current.** This is the exact stratum an external reviewer mis-read as present truth. Specifically: the phrase **"tamper-PROOF"**, **"Schema: v20"**, **"57 ADR files"**, and **"2,800 tests"** below are all SUPERSEDED — current canon is **tamper-EVIDENT** (signed, append-only), **schema v23**, and the counts in the CANON block at the top of this file. Kept verbatim for audit lineage; corrected only here.

The body that follows reflects the B258 baseline. The CANON block at the top of this file is the load-bearing current truth.
