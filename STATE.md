...
- **D7 Content Pipeline** — **IN PROGRESS — Phases A + B + C landing (2026-05-23).** ADR-0088 Proposed. Phase A CLOSED: writer + content_researcher roles + skill manifests + birth scripts; manifest's bare `researcher` entry_agent renamed to `content_researcher` per Decision 2. Phase B CLOSED: style_steward (guardian, GREEN) + `voice_profile_build.v1` (21 tests) + `voice_match_check.v1` (22 tests) + skills + birth script. Phase C CLOSED: editor role (guardian, GREEN) + `format_adapt.v1` (26 tests; deterministic adapter to twitter_thread / linkedin_post / newsletter / blog) + skill manifests (`editing.v1` composing verify_claim + voice_match_check + LLM narrative; `format_adaptation.v1` composing format_adapt + LLM tone polish) + birth script (`birth-editor.command`). Phase D pending: distribution_pilot (YELLOW) + publish_schedule tool + umbrella + cascade wiring (d1→d7, d2→d7).
