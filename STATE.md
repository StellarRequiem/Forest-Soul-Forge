...
- **D9 Learning Coach** — **Phases A + B IN FLIGHT (2026-05-23).** ADR-0089 Phase A + Phase B: mentor + curriculum_designer (researcher, GREEN, Phase A) + assessor (guardian, YELLOW, Phase B) live in trait_tree / genres / constitution_templates / tool_catalog. Four new builtin tools: `curriculum_design.v1` (read_only; 27 tests; deterministic topic-prereq DAG composer), `knowledge_assessment.v1` (read_only; 19 tests; quiz-item envelope generator), `assessment_score.v1` (read_only; 17 tests; strict-match + Jaccard lexical-overlap scorer), `misconception_log.v1` (filesystem; 18 tests; misconception ledger writer — NOT in any role's standard kit because guardian's read_only ceiling rejects filesystem; operator dispatches directly post-review per the D1 knowledge_verifier separation pattern). Four skill manifests (`coaching.v1`, `curriculum_design.v1`, `knowledge_assessment.v1`, `misconception_tracking.v1`). Birth scripts for mentor/curriculum_designer/assessor. Runbook `docs/runbooks/d9-learning-coach-ops.md`. ADR-0089 in Proposed status. Phases C/D queued (C: socratic_partner reuses kit; D: spaced_repetition_pilot YELLOW + 1 tool + cascade wiring + umbrella).
