...
- **D1 Personal Knowledge Forge** — **Phase C closed (2026-05-23).** ADR-0086 Phases A+B+C CLOSED. Roles live: `librarian` (guardian), `prospector` (researcher), `synthesizer` (researcher), `knowledge_verifier` (guardian, YELLOW). Two new builtin tools: `topic_genealogy_build.v1` (28 tests), `knowledge_contradiction_scan.v1` (28 tests; single-agent scope per Decision 3, cross_agent reserved for v0.4). Five skill manifests: `knowledge_curation.v1`, `research_gathering.v1`, `topic_genealogy.v1`, `knowledge_summarize.v1`, `knowledge_contradiction_flag.v1`. Four birth scripts. Runbook Phase A+B+C sections. Phase D queued: daily_knowledge_delta.v1 + cascade wiring + umbrella script.
