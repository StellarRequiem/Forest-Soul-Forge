...
- **D5 Smart Home Brain** — **IN FLIGHT — Phases A+B+C shipped (2026-05-24).** ADR-0091 Phases A + B + C landed. Roles: home_steward (researcher, GREEN) + home_sentinel (guardian, GREEN) [Phase A]; energy_warden (researcher, GREEN) + comfort_optimizer (researcher, GREEN) [Phase B]; routine_composer (actuator, YELLOW) [Phase C]. Four new builtin tools: `energy_anomaly_scan.v1` (Phase B; read_only; z-score per-device classifier across {spike, drift, normal, missing_baseline}; 36 tests), `comfort_recommend.v1` (Phase B; read_only; per-area recommendation composer across scene + temperature + lighting; vacation_mode → set_scene "away" with priority 1; 36 tests), `home_state_snapshot.v1` (Phase C; read_only; deterministic per-room rollup + presence flag + stale_count + anomaly hints; 26 tests), `routine_compose.v1` (Phase C; filesystem; requires_human_approval=True; routine envelope composer + queue writer to data/d5/routine_queue.jsonl across kinds {vacation_mode, morning_sequence, focus_mode, sleep_mode, custom}; 34 tests). Six skill manifests (`home_orchestration.v1`, `home_security.v1`, `energy_optimization.v1`, `comfort_tuning.v1`, `routine_management.v1`, `vacation_mode.v1`). Birth scripts for all five roles. Runbook `docs/runbooks/d5-smart-home-ops.md`. Phase D (cascade + umbrella + live) pending. Substrate-ready: D5 ships without forest-home-assistant connector present (operator supplies `home_state_snapshot` + `baseline_snapshot` attestations; connector ingests them later via the same shape). routine_composer is the only YELLOW role (queue-driven actuation per ADR-0091 Decision 2 — no direct device control in D5; routine_compose.v1 carries requires_human_approval at the tool layer as the load-bearing second safety).
