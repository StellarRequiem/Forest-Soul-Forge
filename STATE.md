...
<!-- CANON:BEGIN — generated by dev-tools/state_canon.py --emit on 2026-06-05 02:17Z. Do not hand-edit; run `--check` to verify against disk. -->

| Surface | Canon (generated 2026-06-05 02:17Z) |
|---:|:---|
| Python LoC (`src/forest_soul_forge/`) | **103,457** |
| ADRs filed | **93** files / **91** unique numbers |
| Builtin tools | **101** |
| Tests | **5,307** `def test_` across **275** files _(structural count — suite GREEN/pass-count is a CI artifact, not asserted here)_ |
...
| As of | **`1c13c8c`** · **721** commits on `main` _(VCS provenance — advances every commit)_ |
| Registry agents | **67 active / 5 archived / 72 total** _(local runtime DB — gitignored, absent in CI)_ |
| Audit chain entries | **63** at `examples/audit_chain.jsonl` _(grows continuously at runtime — snapshot only)_ |
