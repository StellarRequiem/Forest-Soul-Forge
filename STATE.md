...
**Domain rollouts (per ADR-0067 dependency order D4 → D3 → D8 → D1 → D2 → D7 → D9 → D10 → D5 → D6). All 10 domains live as of 2026-05-24:**
...
- **D5 Smart Home Brain** — **CLOSED — fully LIVE, all 5 agents alive (2026-05-24).** ADR-0091 all 4 phases shipped. Roles: home_steward (researcher, GREEN) + home_sentinel (guardian, GREEN) [Phase A]; energy_warden (researcher, GREEN) + comfort_optimizer (researcher, GREEN) [Phase B]; routine_composer (actuator, YELLOW) [Phase C — the only acting role in D5]. Four new builtin tools: `energy_anomaly_scan.v1` (Phase B; read_only; per-device anomaly classifier across {spike, drift, normal, missing_baseline}; 36 tests), `comfort_recommend.v1` (Phase B; read_only; per-area scene + temperature + lighting recommendation composer; 36 tests), `home_state_snapshot.v1` (Phase C; read_only; deterministic per-room rollup + presence flag + stale_count + anomaly hints; 26 tests), `routine_compose.v1` (Phase C; filesystem; requires_human_approval=True; routine envelope composer + queue writer to data/d5/routine_queue.jsonl across kinds {vacation_mode, morning_sequence, focus_mode, sleep_mode, custom}; 34 tests). Seven skill manifests (`home_orchestration.v1`, `home_security.v1`, `energy_optimization.v1`, `comfort_tuning.v1`, `routine_management.v1`, `vacation_mode.v1`, `smart_home.v1` umbrella). Cascade wiring d2→d5 morning_briefing + d2→d5 task_prioritization + d5→d3 home_security + d5→d2 routine_management ACTIVE; d5→d6 + d5→d1 declared INERT. Umbrella birth `dev-tools/birth-d5-smart-home.command`. Runbook `docs/runbooks/d5-smart-home-ops.md`. Substrate-ready: D5 ships without forest-home-assistant connector present (operator supplies `home_state` attestations; connector ingests them later via the same shape). routine_composer is the only YELLOW role (queue-driven actuation per ADR-0091 Decision 2 — no direct device control in D5; routine_compose.v1 carries requires_human_approval at the tool layer as the load-bearing second safety).
- **D6 Personal Finance Guardian** — **CLOSED — fully LIVE, all 5 agents alive (2026-05-24).** ADR-0092 all 4 phases shipped. Roles: budget_analyst (researcher, GREEN) + risk_advisor (guardian, GREEN) [Phase A]; transaction_tracker (researcher, GREEN) + bill_steward (researcher, GREEN) [Phase B]; investment_researcher (researcher, GREEN) [Phase C]. Three new builtin tools: `transaction_categorize.v1` (read_only; 44 tests; deterministic rule-based categorization over operator-supplied transaction batches; first-match-wins; merchant/description/amount predicates), `bill_recurrence_check.v1` (read_only; 46 tests; deterministic monthly/quarterly/annual recurrence detection with operator-tolerable day_drift + next-due projection + missing-cycle flagging), `investment_compare.v1` (read_only; 45 tests; deterministic side-by-side comparison composer with per-dimension winners + per-option deltas; tool itself refuses to emit a "recommended option" field — operator decides per manifest's info-only floor). Six skill manifests (`budget_analysis.v1`, `risk_analysis.v1`, `transaction_monitoring.v1`, `bill_management.v1`, `investment_research.v1`, `finance_brain.v1` umbrella). Cascade wiring d5→d6 energy_optimization → transaction_monitoring + d6→d2 bill_management → reminder + d6→d8 tax_season_summary → compliance_scan ACTIVE; d2→d6/d6→d1/d6→d3 declared INERT. Umbrella birth `dev-tools/birth-d6-finance.command`. Runbook `docs/runbooks/d6-finance-ops.md`. D6's hard rule: NEVER executes transactions (manifest contract). ALL D6 roles are GREEN — first all-GREEN domain in the rollout, because the actuating surface (bank, broker, bill-pay) is operator-only by manifest contract; there is no actuator lane to define. **D6 closes the cross-domain rollout per ADR-0067 — all 10 domains live (D4 → D3 → D8 → D1 → D2 → D7 → D9 → D10 → D5 → D6).**
