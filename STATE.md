...
- **D7 Content Pipeline** — **IN PROGRESS — Phase A + B landing (2026-05-23).** ADR-0088 Proposed. Phase A CLOSED: writer + content_researcher roles + skill manifests (`draft_writing.v1`, `content_research.v1`) + birth scripts (`birth-writer.command`, `birth-content-researcher.command`); manifest's bare `researcher` entry_agent renamed to `content_researcher` per Decision 2. Phase B CLOSED: style_steward role (guardian, GREEN) + two new builtin tools `voice_profile_build.v1` (21 tests; deterministic stylometric profiler over operator writing samples) + `voice_match_check.v1` (22 tests; scores drafts against profile + flags drift with span pointers) + skill manifests (`voice_profile_build.v1`, `voice_matching.v1`) + birth script (`birth-style-steward.command`). Phases C–D pending: C = editor + format_adapt tool, D = distribution_pilot (YELLOW) + publish_schedule tool + umbrella + cascade wiring (d1→d7, d2→d7).
- **D9/D10/D5/D6** — upstream; not started.
