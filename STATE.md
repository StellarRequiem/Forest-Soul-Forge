...
Last updated: 2026-05-04, post-Burst 91 (ADR-0041 Set-and-Forget Orchestrator complete except T4 scenario task type — T1 design Burst 85; T2 runtime + lifespan Burst 86; T3 tool_call task runner + audit emit Burst 89; T5 SQLite v13 persistence Burst 90; T6 operator control endpoints Burst 91). v0.3.0 shipped 2026-05-03 (ADR-0036 Verifier Loop + ADR-0040 Trust-Surface Decomposition arcs both feature-complete). v0.4.0-rc tagged 2026-05-04 locking the production-grade tool_call-only scheduler as a checkpoint before T4 scenario-runner work. Test count 2072 → 2129 across the v0.4-rc arc (+57 across Bursts 86-91, zero regressions). v0.2.0 shipped 2026-05-02 (Phase G.1.A close — 10 programming primitives completing the SW-track change loop). v0.1.2 shipped 2026-05-01 absorbing SarahR1's review (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30 (audit + hardening). See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
...
| Tests (passing) | **2129** (was 1567 at v0.1.2; +562 net — +401 across the v0.2 arc, +104 across the v0.3 ADR-0036 arc Bursts 65-70, +57 across the v0.4-rc ADR-0041 arc Bursts 86-91) |
| ADRs filed | 38 files / 36 unique numbers (`ADR-0001` → `ADR-0041`, with gaps 0009-0015; ADR-003X open-web + ADR-003Y conversation runtime drafts; ADR-0021-am + ADR-0027-am amendments + ADR-0038 companion harm model all Accepted in v0.1.2; ADR-0035 Persona Forge + ADR-0037 Observability dashboard Proposed for v0.3; **ADR-0036 Verifier Loop feature-complete in v0.3 (T1+T2+T3a+T3b+T5+T6+T7 shipped; T4 scheduled-task substrate now closed by ADR-0041)**; **ADR-0039 Distillation Forge / Swarm Orchestrator Proposed for v0.4**; **ADR-0040 Trust-surface decomposition rule Accepted (T1+T2+T3+T4 all shipped 2026-05-02)**; **ADR-0041 Set-and-Forget Orchestrator Accepted, T1+T2+T3+T5+T6 shipped Bursts 85-91; T4 scenario task type still outstanding**) |
...
| Audit event types | **62** (54 pre-v0.3 + verifier_scan_completed from ADR-0036 T3b/T5 + 7 ADR-0041 scheduler events Bursts 86-91: scheduled_task_dispatched / completed / failed / circuit_breaker_tripped / circuit_breaker_reset / enabled / disabled) |
| Frontend modules (vanilla JS) | 22 (was 18 + chat.js + cleanup) |
| `.command` operator scripts | **104** at repo root (88 at v0.3.0 + 16 across the v0.4-rc arc Bursts 85-91 — commit-burst85 through 91 + verify-burst86 + open-in-chrome + helpers). Includes start/stop/reset + ~25 live-tests + ~58 commit-burst* + dist/build + ops scripts. |
...
| Total commits on `main` | **247** (234 at v0.3.0 + 13 across the v0.4-rc arc: ADR-0041 design + scheduler runtime + tool_call runner + persistence + control endpoints + 4 frontend hotfixes + roadmap doc + audit-pass fixes) |
...
| 0041 | Set-and-Forget Orchestrator | **Accepted** — T1 design (Burst 85), T2 runtime + lifespan (Burst 86), T3 tool_call task type + audit emit (Burst 89), T5 SQLite v13 persistence (Burst 90), T6 operator control endpoints — trigger / enable / disable / reset (Burst 91). T4 scenario task type still outstanding; production-grade tool_call-only scheduler tagged as v0.4.0-rc 2026-05-04. |
