...
Last updated: 2026-05-11, post-Burst 212 (real uninstall endpoints for forged skills + tools). Bursts 200-212 layered a **natural-language Forge UI arc** onto the v0.6 baseline. The arc made the existing Skill Forge (ADR-0031) and Prompt-Tool Forge (ADR-0058) operator-usable end-to-end: a non-developer can type a description into the SoulUX modal, watch the LLM propose a YAML manifest or spec, review the preview, install, and dispatch the artifact — all under audit. Three filed ADRs back the arc: **ADR-0057 Skill Forge UI** (the modal + /skills/forge + /skills/install routes), **ADR-0058 Tool Forge UI — prompt-template path** (the prompt_template_tool.v1 generic class + /tools/forge + /tools/install), **ADR-0059 Catalog-Aware Propose** (the LLM gets the live tool catalog injected into the propose prompt so it can't hallucinate tool names). Concrete milestones during the arc: **B201** skill-forge UI shipped; **B202** tool-forge UI shipped; **B204** catalog injection landed in both forge engines + an unknown_tools_referenced 422 path on install; **B205** "Forged proposals" subsection on the Approvals tab; **B206** test fixture migration to the B148 X-FSF-Token security gate (62 failing tests → 0, suite restored to 2,738 passing); **B207** skill-forge failure-mode diagnostics (forge.log + manifest_raw.yaml written BEFORE parse so a quarantine artifact survives even when YAML parse fails) + frontend structured 422 rendering with collapsible forge.log excerpt; **B208** prompt v3 explicit reference-scope rules teaching the LLM to use `${inputs.X}` not bare `${X}` + ManifestError text capture in forge.log; **B209** tool-forge diagnostic parity with B207+B208 (fence stripping + quarantine + log capture); **B210** prompt_tool_forge propose-quality fix — explicit runtime-model section explaining the template gets sent to ANOTHER LLM at execute() time, with WRONG/RIGHT examples; **B211** staged-list endpoints filter installed artifacts so the Approvals "Forged proposals" panel shows pending only; **B212** real uninstall endpoints (DELETE /skills/installed + /tools/installed with audit events). Three artifacts forged end-to-end via the UI pipeline during the arc and live-dispatched: `summarize_audit_chain.v1` skill (seq 7498/7511), `translate_to_french.v1` tool (seq 7537, dispatched and produced "Bonjour, comment allez-vous aujourd'hui ?" at seq 7554), plus the discarded `text_to_bullet_points.v1` (kept as B210-era evidence of the broken-template pattern that B210 then fixed). The natural-language Forge UI arc closes ADR-0057 + ADR-0058 at the operator-usable threshold. v0.5 ADRs remain canonical: **ADR-0042**, **ADR-0043**, **ADR-0045**. Test count post-B212: **2,738 unit + integration passing** (sandbox baseline; the +140 over the B199 count of 2,598 comes from B201/B202 forge tests + B206 fixture migration restoring previously-blocked tests). Latest tag still **v0.5.0** (2026-05-04). v0.6 has not yet been tagged — gated on integrator validation per ADR-0044 P7. v0.4.0 shipped 2026-05-04 (ADR-0041). v0.3.0 shipped 2026-05-03 (ADR-0036 + ADR-0040). v0.2.0 shipped 2026-05-02 (Phase G.1.A). v0.1.2 shipped 2026-05-01 (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30. See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
...
| | |
|---:|:---|
| Source LoC (Python) | **56,113** across `src/forest_soul_forge/` (was 50,289 at Burst 124; +5,824 across Bursts 125-199 — assistant chat router, hardware binding, plugin grants, procedural shortcut dispatcher steps, experimenter mode kit clamp, scheduler runtime hardening). |
| Tests (passing) | **2,738 unit + integration** (sandbox baseline post-B212; was 2,598 at Burst 199, +140 across Bursts 200-212 = B201/B202 forge test additions + B206 fixture migration restoring 62 previously-blocked tests). Plus **conformance suite** at `tests/conformance/` (Burst 130, ADR-0044 P4). Conformance is HTTP-only — external integrators run it against their own Forest-kernel build via `pip install "forest-soul-forge[conformance]" && pytest tests/conformance/`. Tests organized by spec section (§1 dispatch / §2 audit chain / §3 plugin manifest / §4 constitution / §5 HTTP API). |
| ADRs filed | **56 files / 54 unique numbers** (`ADR-0001` → `ADR-0059`, with gaps 0009-0015; ADR-003X open-web + ADR-003Y conversation runtime drafts; ADR-0021-am + ADR-0027-am amendments + ADR-0038 all Accepted in v0.1.2). v0.5/v0.6 active ADRs: **ADR-0042 v0.5 Product Direction** (T5 signing/auto-updater gated on Apple Developer decision); **ADR-0043 MCP-First Plugin Protocol** (T1-T5 + follow-ups #1/#2/#3 shipped; #4 plugin_secret_set deferred); **ADR-0044 Kernel Positioning + SoulUX**; **ADR-0045 Agent Posture / Trust-Light System**; **ADR-0046 License + Governance**; **ADR-0047 Persistent Assistant Chat** (6/6 tranches shipped Bursts 135-158); **ADR-0048 Computer-Control Allowance** (drafted, partial implementation); **ADR-0049/0050/0051 Per-event signatures + Encryption-at-rest + Per-tool subprocess sandbox** (drafted/partial); **ADR-0052 Pluggable Secrets Storage** + **ADR-0053 Per-Tool Plugin Grants** (Bursts 159-178); **ADR-0054 Procedural-Shortcut Dispatch** (T1-T6 + T5b shipped Bursts 161-195 — substrate DEFAULT OFF until operator opts in via `procedural_shortcut_enabled`); **ADR-0055 Agentic Marketplace** (drafted); **ADR-0056 Experimenter Agent** (E1-E7 shipped Bursts 188-197); **ADR-0057 Skill Forge UI** (operator-usable end-to-end post-B208); **ADR-0058 Tool Forge UI — prompt-template path** (operator-usable end-to-end post-B210); **ADR-0059 Catalog-Aware Propose** (forge prompts inject the live tool catalog so the LLM can't hallucinate tool names). |
...
| Skill manifests | **26 shipped** in `examples/skills/` (canonical authored set, unchanged Bursts 125-199). **0 currently installed** in `data/forge/skills/installed/` — the install dir was reset post-2026-05-03 (backup at `data/forge/skills/installed.bak.20260503-101637`). Skills loaded at runtime from examples/ via the catalog regardless; agents have invoked skills 32× since reset (audit chain `skill_invoked` count). Operator re-installs via `fsf skill install <manifest>` if persistence is wanted. |
...
| Alive agents in registry | **23** (post-B214 swarm re-acceptance). Non-swarm 14: assistant, dashboard_watcher, experimenter, incident_correlator, knowledge_consolidator, operator_companion, paper_summarizer, signal_listener, software_engineer (×2), status_reporter, system_architect, translator (B210 sandbox), vendor_research. **Plus all 9 blue-team agents alive post-B214**: patch_patrol, gatekeeper, log_lurker, anomaly_ace, net_ninja, response_rogue, zero_zero, vault_warden, deception_duke. ADR-0033 Security Swarm re-acceptance evidence at seqs 7593-7643 with 3 `agent_delegated` hops at 7612/7621/7630. The swarm is alive but NOT continuously running — that's gated on the Mac mini 24/7 launchd recipe (B216 queued). |
| Installed skills | **26** post-B214 (was 2 post-B212; +24 from the canonical ADR-0033 swarm skill set reloaded during the re-acceptance smoke — `morning_sweep`, `investigate_finding`, `contain_incident`, `key_audit`, `daily_patch_sweep`, `device_inventory`, etc.). Plus the 2 from the natural-language Forge UI arc: `smoke_blurb.v1`, `summarize_audit_chain.v1`. |
| Installed forged tools | **1** (`translate_to_french.v1`) — live-dispatched successfully at seq 7554 producing "Bonjour, comment allez-vous aujourd'hui ?" |
| Audit event types | **73** in `KNOWN_EVENT_TYPES` post-B212 (was 71 post-B199 — +2 for the B212 uninstall path: `forge_skill_uninstalled` + `forge_tool_uninstalled`). |
| Frontend modules (vanilla JS) — **SoulUX-distribution metric, not kernel** | **26** (was 22 at Burst 124; +4 across Bursts 125-199 for ADR-0047 chat, ADR-0054 thumbs UI, ADR-0056 cycles pane, ADR-0048 allowances pane). Per ADR-0044 the frontend is part of the SoulUX flagship distribution; the kernel runs without it. See `docs/runbooks/headless-install.md`. |
| `.command` operator scripts | **59 at repo root** (was 43 at Burst 128; +16 across Bursts 125-199 for diagnostics + force-restart-daemon + scenario tests + chain-fork-check) plus **172 archived** under `dev-tools/commit-bursts/` (was 100 at Burst 128; +72 per-burst commit scripts) plus **16 in `dev-tools/`** (drift / lock-clearing / rebuild helpers). Convention since Burst 128: one-shot commit scripts land in the archive folder; reusable helpers live in `dev-tools/`. |
...
| Total commits on `main` | **369** post-B212 (was 358 post-B199; +11 across Bursts 200-212 — natural-language Forge UI arc + uninstall endpoints + STATE refresh burst). |
...
| Live audit chain path | **`examples/audit_chain.jsonl`** (per `daemon/config.py` `audit_chain_path` default — NOT `data/audit_chain.jsonl` which is the dev fixture). Override via `FSF_AUDIT_CHAIN_PATH`. **~7,581 entries** on 2026-05-11 (was 3,840 on 2026-05-08; +3,741 across three days — still dominated by `dashboard_watcher`'s 5-minute scheduled polling, plus new forge/install/dispatch events from the Bursts 200-212 arc). |
