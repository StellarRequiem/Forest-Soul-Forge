...
Last updated: 2026-05-08, post-Burst 199 (audit-chain fork fix — 3-layer defense). Bursts 125-199 were a 75-burst v0.6 development arc layered onto the v0.5.0 baseline, organized into seven major tracks: **ADR-0047 Persistent Assistant Chat** (Bursts 135-158, 6 tranches) shipping the Sage assistant agent for operator-companion roles; **ADR-0048 Computer-Control Allowance** drafted for governance relaxation; **ADR-0049/0050/0051 hardware binding** for local-first identity; **ADR-0052 Pluggable Secrets Storage** + **ADR-0053 Per-Tool Plugin Grants** (Bursts 159-178) extending ADR-0043; **ADR-0054 Procedural-Shortcut Dispatch** (Bursts 161-195, 6 tranches T1-T6 plus T5b chat-tab thumbs UI) — substrate landed but DEFAULT OFF until operator opt-in via `procedural_shortcut_enabled`; **ADR-0055 Agentic Marketplace** drafted; **ADR-0056 Experimenter Agent** (Bursts 188-197, 7 tranches E1-E7) bringing self-recursive system improvement online with the Smith experimenter agent. Net effect: **13 active agents in the registry** (operator_companion, Forge_AuditTight_01 SW-track, Sage assistant, Kraine system_architect, Victor knowledge_consolidator, chaz software_engineer, Smith experimenter, plus the 6-agent 24/7 specialist stable: dashboard_watcher / signal_listener / incident_correlator / paper_summarizer / vendor_research / status_reporter). **B198** fixed a latent scheduler `result_digest` method-vs-value bug + shipped `force-restart-daemon.command` operator script. **B199** fixed an architectural race where `chain.append` had no internal lock and two startup-time call sites (`build_plugin_runtime` and scheduler `_dispatch`) bypassed `app.state.write_lock` — historical chain forks at seqs 3728/3735-3738/3740 documented in `docs/audits/2026-05-08-chain-fork-incident.md`; chain now self-protected by an internal `RLock`, both bypasses fixed, verifier `KNOWN_EVENT_TYPES` updated 57→71, and a new `scan_for_forks()` exhaustive verifier added. v0.5 ADRs remain canonical: **ADR-0042**, **ADR-0043**, **ADR-0045**. Test count post-B199: **2,598 unit + integration passing** (sandbox baseline; +212 since v0.5.0 across 75 bursts). Latest tag still **v0.5.0** (2026-05-04). v0.6 has not yet been tagged — gated on integrator validation per ADR-0044 P7. v0.4.0 shipped 2026-05-04 (ADR-0041). v0.3.0 shipped 2026-05-03 (ADR-0036 + ADR-0040). v0.2.0 shipped 2026-05-02 (Phase G.1.A). v0.1.2 shipped 2026-05-01 (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30. See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
...
| Tests (passing) | **2,598 unit + integration** (sandbox baseline post-B199; was 2,386 at Burst 124, +212 across 75 bursts) + **conformance suite** at `tests/conformance/` (Burst 130, ADR-0044 P4). Conformance is HTTP-only — external integrators run it against their own Forest-kernel build via `pip install "forest-soul-forge[conformance]" && pytest tests/conformance/`. Tests organized by spec section (§1 dispatch / §2 audit chain / §3 plugin manifest / §4 constitution / §5 HTTP API). |
| ADRs filed | **53 files / 51 unique numbers** (`ADR-0001` → `ADR-0056`, with gaps 0009-0015; ADR-003X open-web + ADR-003Y conversation runtime drafts; ADR-0021-am + ADR-0027-am amendments + ADR-0038 all Accepted in v0.1.2). v0.5/v0.6 active ADRs: **ADR-0042 v0.5 Product Direction** (T5 signing/auto-updater gated on Apple Developer decision); **ADR-0043 MCP-First Plugin Protocol** (T1-T5 + follow-ups #1/#2/#3 shipped; #4 plugin_secret_set deferred); **ADR-0044 Kernel Positioning + SoulUX**; **ADR-0045 Agent Posture / Trust-Light System**; **ADR-0046 License + Governance**; **ADR-0047 Persistent Assistant Chat** (6/6 tranches shipped Bursts 135-158); **ADR-0048 Computer-Control Allowance** (drafted, partial implementation); **ADR-0049/0050/0051 Per-event signatures + Encryption-at-rest + Per-tool subprocess sandbox** (drafted/partial); **ADR-0052 Pluggable Secrets Storage** + **ADR-0053 Per-Tool Plugin Grants** (Bursts 159-178); **ADR-0054 Procedural-Shortcut Dispatch** (T1-T6 + T5b shipped Bursts 161-195 — substrate DEFAULT OFF until operator opts in via `procedural_shortcut_enabled`); **ADR-0055 Agentic Marketplace** (drafted); **ADR-0056 Experimenter Agent** (E1-E7 shipped Bursts 188-197). |
...
| Trait roles | **44** (5 original + 9 swarm + 3 SW-track + 1 verifier_loop + 24 Burst 124 expansion + **2 added in Bursts 125-199**: ADR-0047 `assistant` for the Sage agent, ADR-0056 `experimenter` for the Smith agent). |
| Audit event types | **71** in `KNOWN_EVENT_TYPES` post-B199 (was 57 pre-B199 — +14 events the catalog had drifted out of sync with: plugin_installed, scheduled_task_dispatched/completed/failed/circuit_breaker_tripped/circuit_breaker_reset, agent_posture_changed, out_of_triune_attempt, hardware_bound/mismatch/unbound, agent_plugin_granted, governance_relaxed, task_caps_set). Pre-B199 the verifier silently logged these as forward-compat warnings on every chain walk. |
...
| Total commits on `main` | **358** (273 at Burst 115 + 85 across Bursts 116-199, will be 359 once this drift-refresh burst lands). |
...
| Live audit chain path | **`examples/audit_chain.jsonl`** (per `daemon/config.py` `audit_chain_path` default — NOT `data/audit_chain.jsonl` which is the dev fixture). Override via `FSF_AUDIT_CHAIN_PATH`. **~3,840 entries** on 2026-05-08 (was 1,121 on 2026-05-05; +2,719 from the 24/7 specialist stable's scheduled_task_dispatched events — `dashboard_watcher` polls every 5 minutes and is far the dominant emitter). |
