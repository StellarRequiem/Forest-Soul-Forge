...
- **D8 Compliance Auditor** — ROLLING OUT (ADR-0085, started 2026-05-22). Phase A IN PROGRESS — audit_archivist + evidence_collector roles + skills (`evidence_collection.v1` + `long_term_archival.v1`) + birth scripts shipped; AuditArchivist-D8 + EvidenceCollector-D8 births pending operator approval queue. Phases B (compliance_scanner + framework_check.v1), C (policy_enforcer + policy_lint.v1), D (report_generator + audit_packet_generate.v1 + cascade) pending.
