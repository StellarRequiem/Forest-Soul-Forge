...
- **D2 Daily Life OS** — **Phases A+B IN FLIGHT (2026-05-23).** ADR-0087 Proposed. Phase A roles: coordinator (researcher, GREEN) + inbox_triager (communicator, GREEN). Phase B role: time_steward (actuator, YELLOW). Two new builtin tools in Phase B: `schedule_reminder.v1` (filesystem; 23 tests) + `calendar_block.v1` (external; 28 tests; graceful degrades when forest-calendar absent). Four skills so far: `daily_orchestration.v1`, `inbox_triage.v1`, `schedule_reminder.v1`, `calendar_management.v1`. Birth scripts: `dev-tools/birth-coordinator.command`, `dev-tools/birth-inbox-triager.command`, `dev-tools/birth-time-steward.command`. Runbook `docs/runbooks/d2-daily-life-os-ops.md`. Phases C/D queued: task_prioritizer with task_rank.v1; reflector with decision_journal_compile.v1. Cascade d1→d2 morning_briefing activates at Phase D.
