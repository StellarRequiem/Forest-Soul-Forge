...
| Tests (passing) | **2386 unit** + **conformance suite** at `tests/conformance/` (Burst 130, ADR-0044 P4). Unit suite unchanged Bursts 116-124. Conformance is HTTP-only — external integrators run it against their own Forest-kernel build via `pip install "forest-soul-forge[conformance]" && pytest tests/conformance/`. Tests organized by spec section (§1 dispatch / §2 audit chain / §3 plugin manifest / §4 constitution / §5 HTTP API / §6 CLI / §7 schema). |
