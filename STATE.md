...
<!-- CANON:BEGIN — generated by dev-tools/state_canon.py --emit on 2026-06-04 05:57Z. Do not hand-edit; run `--check` to verify against disk. -->

| Surface | Canon (generated 2026-06-04 05:57Z) |
|---:|:---|
| Python LoC (`src/forest_soul_forge/`) | **102,912** |
| ADRs filed | **92** files / **90** unique numbers |
| Builtin tools | **101** |
| Tests | **5,281** `def test_` across **271** files _(structural count — suite GREEN/pass-count is a CI artifact, not asserted here)_ |
...
| As of | **`ff888d6`** · **703** commits on `main` _(VCS provenance — advances every commit)_ |
| Registry agents | **67 active / 5 archived / 72 total** _(local runtime DB — gitignored, absent in CI)_ |
| Audit chain entries | **27,913** at `examples/audit_chain.jsonl` _(grows continuously at runtime — snapshot only)_ |
