...
Last updated: 2026-05-04, post-Burst 94 (v0.4.0 final ready to tag). ADR-0041 Set-and-Forget Orchestrator implementation-complete: T1 design Burst 85, T2 runtime + lifespan Burst 86, T3 tool_call task runner + audit emit Burst 89, T4 scenario task type runtime Burst 93, T5 SQLite v13 persistence Burst 90, T6 operator control endpoints Burst 91, FizzBuzz YAML scenario port Burst 94 (closes Burst 81 P1). v0.4.0-rc tagged 2026-05-04 with the tool_call-only scheduler as a checkpoint; v0.4.0 final supersedes it after T4 + the FizzBuzz YAML port landed. v0.3.0 shipped 2026-05-03 (ADR-0036 Verifier Loop + ADR-0040 Trust-Surface Decomposition arcs both feature-complete). Test count 2072 → 2177 across the v0.4 arc (+105 across Bursts 86-94, zero regressions). v0.2.0 shipped 2026-05-02 (Phase G.1.A close — 10 programming primitives completing the SW-track change loop). v0.1.2 shipped 2026-05-01 absorbing SarahR1's review (ADR-0027-am + ADR-0021-am + ADR-0038). v0.1.1 shipped 2026-04-30 (audit + hardening). See [CHANGELOG.md](CHANGELOG.md) and [CREDITS.md](CREDITS.md) for the full attribution + ledger.
...
| Tests (passing) | **2177** (was 1567 at v0.1.2; +610 net — +401 across the v0.2 arc, +104 across the v0.3 ADR-0036 arc Bursts 65-70, +105 across the v0.4 ADR-0041 arc Bursts 86-94) |
| ADRs filed | 38 files / 36 unique numbers (`ADR-0001` → `ADR-0041`, with gaps 0009-0015; ADR-003X open-web + ADR-003Y conversation runtime drafts; ADR-0021-am + ADR-0027-am amendments + ADR-0038 companion harm model all Accepted in v0.1.2; ADR-0035 Persona Forge + ADR-0037 Observability dashboard Proposed for v0.3; **ADR-0036 Verifier Loop feature-complete in v0.3 (T1+T2+T3a+T3b+T5+T6+T7 shipped; T4 scheduled-task substrate closed by ADR-0041 T3)**; **ADR-0039 Distillation Forge / Swarm Orchestrator Proposed for v0.4**; **ADR-0040 Trust-surface decomposition rule Accepted (T1+T2+T3+T4 all shipped 2026-05-02)**; **ADR-0041 Set-and-Forget Orchestrator Accepted, all 5 implementation tranches shipped Bursts 85-93 plus FizzBuzz YAML scenario port Burst 94 — v0.4.0 ready**) |
...
| `.command` operator scripts | **107** at repo root (88 at v0.3.0 + 19 across the v0.4 arc Bursts 85-94 — commit-burst85 through 94 + tag-v0.4.0-rc + verify-burst86 + open-in-chrome + helpers). Includes start/stop/reset + ~25 live-tests + ~61 commit-burst* + dist/build + ops scripts. |
...
| Total commits on `main` | **250** (234 at v0.3.0 + 16 across the v0.4 arc: ADR-0041 design + scheduler runtime + tool_call runner + persistence + control endpoints + scenario task type + FizzBuzz YAML + 4 frontend hotfixes + roadmap doc + audit-pass fixes + v0.4.0-rc/v0.4.0 docs/tags) |
