...
- **D6 Personal Finance Guardian** — **IN FLIGHT — Phase A shipped (2026-05-24).** ADR-0092 Phase A landed: budget_analyst (researcher, GREEN) + risk_advisor (guardian, GREEN). No new builtin tools (both roles reuse existing kit — analyst's burn-rate composition + advisor's anti-recommendation arbitration are LLM-driven over recall + write). Two skill manifests (`budget_analysis.v1`, `risk_analysis.v1`). Birth scripts `dev-tools/birth-budget-analyst.command` + `dev-tools/birth-risk-advisor.command`. Runbook `docs/runbooks/d6-finance-ops.md`. Phases B (transaction_tracker + bill_steward + `transaction_categorize.v1` + `bill_recurrence_check.v1`), C (investment_researcher + `investment_compare.v1`), D (cascade + umbrella + live) pending. D6's hard rule: NEVER executes transactions (manifest contract). ALL D6 roles are GREEN — first all-GREEN domain in the rollout, because the actuating surface (bank, broker, bill-pay) is operator-only by manifest contract; there is no actuator lane to define. Closes the rollout per ADR-0067 (D6 is the LAST domain).
