...
class Registry:
    ...
    # Audit emission lives in the calling layer (dispatcher / endpoint
    # for writes, ToolContext.secrets accessor for reads) so the
    # Registry stays a pure SQL surface. The audit chain receives
    # secret_set / secret_revealed / secret_blocked / secret_revoked
    # events with the (instance_id, name) pair only — never the value.
    ...
    def set_secret(
        ...
        *,
        ...
        ``master_key`` is required — Registry never reads the env var
        ...
        with _transaction(self._conn):
            self._conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, NULL);
                """,
                (instance_id, name, ciphertext, nonce, when or _utc_now_iso()),
            )
        ...
        *,
        ...
        with _transaction(self._conn):
            self._conn.execute(
                ...
                (when or _utc_now_iso(), instance_id, name),
            )
        return plaintext
