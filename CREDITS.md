...
## Internal-research catalysts

The following ADRs were filed against design proposals + literature surveys
generated by Alex (orchestrator) using Claude sessions for research, NOT
external contributors. Documented here for traceability — when a future
ADR cites "the internal-research analysis (date)" or "the bibliography
survey (date)" the trail of where the design space pressure came from
remains visible.

### Hierarchical multi-agent / Distillation Forge research (2026-05-01)

Two parallel research products on the same date:

1. A design analysis proposing the 1-large + N-small hierarchical
   pattern with realistic M4-Mac-Mini hardware estimates and an
   implementation sketch (Distillation Forge skill + Swarm
   Orchestrator genre).
2. A literature survey grounding the proposal against published
   research, providing the bibliography that lands in
   ADR-0039's appendix.
...
| ADR / doc | Adoption shape |
|---|---|
| [ADR-0039 — Distillation Forge + Swarm Orchestrator](docs/decisions/ADR-0039-distillation-forge-swarm-orchestrator.md) | Captures the full design space. Architectural rule (§4 anti-god-object discipline) is the orchestrator's explicit constraint added at design time. Constitutional AI (Bai et al. 2022) + Orca (Mukherjee et al. 2023) + AutoGen (Wu et al. 2023) are load-bearing citations from the bibliography survey. Distilled-agents-have-new-DNA semantics (§5), MLX-only dependency commitment (§6), Persona Forge interaction (§7), and Verifier Loop sequence dependency (§8) are FSF-specific work resolving load-bearing questions the analysis didn't address. **Status: Proposed (v0.4 candidate).** |
| [docs/audits/2026-05-01-fsf-dispatch-overhead-benchmark-plan.md](docs/audits/2026-05-01-fsf-dispatch-overhead-benchmark-plan.md) | Sketches the benchmark Burst that ADR-0039 §10 depends on for realistic throughput modeling. Pre-implementation cost: ~1 substantive session + 1 measurement session. **Status: plan.** |
